...
function statWizardNext(step) {
  if (step === 1) {
    const type = getWizardValue('sw-outcome-type');
    if (!type) {
      alert('主要アウトカムのタイプを選択してください。');
      return;
    }
  } else if (step === 2) {
    const cmp = getWizardValue('sw-compare');
    if (!cmp) {
      alert('何と何を比べるかを選択してください。');
      return;
    }
  }
  document.getElementById(`stat-wizard-step-${step}`).classList.add('hidden');
  document.getElementById(`stat-wizard-step-${step+1}`).classList.remove('hidden');
}

function statWizardBack(step) {
  document.getElementById(`stat-wizard-step-${step}`).classList.add('hidden');
  document.getElementById(`stat-wizard-step-${step-1}`).classList.remove('hidden');
}

function statWizardFinish() {
  const type  = getWizardValue('sw-outcome-type');
  const cmp   = getWizardValue('sw-compare');
  const adj   = getWizardValue('sw-adjust') || 'none';
  const trial = getWizardValue('sw-trial') || 'exploratory';

  if (!type || !cmp) {
    alert('ステップ1〜3まで順に選択してください。');
    return;
  }
  generateStatPlanFromWizard({ type, cmp, adj, trial });
}

function generateStatPlanFromWizard(opts) {
  const outcomeType = opts.type;
  const trialType   = opts.trial;
  const adjust      = opts.adj;
  const cmp         = opts.cmp;
  ...
  let text = `【統計解析計画（ひな形）】\n\n`;
  if (theme)  text += `研究テーマ：${theme}\n\n`;
  if (design) text += `想定研究デザイン：${design}\n\n`;

  text += `1. 解析集団\n`;
  text += `原則としてintention-to-treat集団を主要解析対象とする。必要に応じてper-protocol集団による感度解析を実施する。\n\n`;

  text += `2. ベースライン比較\n`;
  text += `群間の背景因子は、連続変数はt検定、カテゴリ変数はカイ二乗検定（必要に応じてFisherの正確確率検定）を用いて記述する。\n\n`;

  text += `3. 主要評価項目の解析\n`;
  if (outcomeType === 'time') {
    text += `主要評価項目はKaplan–Meier法で生存曲線を推定し、log-rank検定で群間比較を行う。治療群を説明変数、年齢・PS等を共変量としたCox比例ハザードモデルでハザード比と95％信頼区間を推定する。\n\n`;
  } else if (outcomeType === 'continuous') {
    if (cmp === 'two-arm' || cmp === 'single') {
      text += `主要評価項目は群別の平均値・標準偏差を算出し、群間比較にはt検定を用いる。ベースライン値の差を調整する場合は共分散分析（ANCOVA）を用いる。\n\n`;
    } else if (cmp === 'paired') {
      text += `主要評価項目は前後差を算出し、対応のあるt検定により平均差の検定を行う。必要に応じて線形混合効果モデルで経時変化を解析する。\n\n`;
    }
  } else if (outcomeType === 'binary') {
    text += `主要評価項目は群別の割合を算出し、カイ二乗検定またはFisherの正確確率検定で群間比較を行う。`;
    if (adjust === 'yes') {
      text += `さらにロジスティック回帰解析により、治療群と共変量（年齢、PS等）を説明変数としてオッズ比と95％信頼区間を推定する。\n\n`;
    } else {
      text += `必要に応じてロジスティック回帰解析でオッズ比と95％信頼区間を推定する。\n\n`;
    }
  } else {
    text += `主要評価項目の型を選択すると、推奨解析方法のひな形が自動生成されます。\n\n`;
  }

  text += `4. 試験タイプに応じた解釈\n`;
  if (trialType === 'superiority') {
    text += `本研究は優越性試験として位置づけ、有意水準5％（両側）で治療群の優越性を検証する。\n\n`;
  } else if (trialType === 'noninferiority') {
    text += `本研究は非劣性試験として位置づけ、事前に設定した非劣性マージンを用いて非劣性を検証する。95％信頼区間の下限がマージンを上回ることを確認する。\n\n`;
  } else {
    text += `本研究は探索的試験として位置づけ、得られた効果量と信頼区間を記述的に解釈する。\n\n`;
  }

  text += `5. 統計学的有意性\n`;
  text += `有意水準は原則5％（両側）とし、多重比較が存在する場合は階層化仮説検定や補正法の適用を検討する。\n\n`;

  text += `6. 使用ソフトウェア\n`;
  text += `統計解析にはRまたは同等の統計ソフトウェアを用いる。\n`;

  document.getElementById('stat-plan-box').textContent = text;
}
